...
import com.marklogic.appdeployer.util.ResourceHashManager;
 ...
 */
public abstract class AbstractIncrementalResourceCommand extends AbstractResourceCommand {
	...
	private static ResourceHashManager resourceHashManager = new ResourceHashManager();
    ...
    protected abstract void skippingResource(CommandContext context, File f);
    protected abstract Boolean incrementalMode();
	...
	protected void processExecuteOnResourceDir(CommandContext context, File resourceDir) {
		if (resourceDir.exists()) {
			...
			for (File f : listFilesInDirectory(resourceDir, context)) {
				...
				if (!incrementalMode() || resourceHashManager.hasFileBeenModifiedSinceLastDeployed(f)) {
					if (logger.isInfoEnabled()) {
					...
					resourceHashManager.saveLastDeployedHash(f);
				} else {
					if (logger.isInfoEnabled()) {
						logger.info("File has NOT been modified.");
					}
					skippingResource(context, f);
				}
			}
		} else {
